...
You can install the latest version of `uktrade` from this GitHub repository using `devtools` or `remotes`:
...
## :page_facing_up: Example using OTS and RTS data
...
Loading aggregate data (such as all spirits, HS4 code 2208) is possible too (in the background, this is loading commodity codes larger than or equal to 22080000, and less than or equal to 22089999):
...
Similarly, specifying `commodity = 0` or `commodity = NULL` will load all commodities (this may take considerable time). For example, we can load all exports to Australia in January 2019:
...
Loading all UK regional trade of 2-digit SITC Division '00 - Live animals other than animals of division 03' for 2019 is done by specifying `sitc = 00` (which means leading zeros are removed):

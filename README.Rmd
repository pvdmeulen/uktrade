...
Similarly, specifying `commodity = NULL` will load all commodities (this may take considerable time). For example, we can load all exports to Australia in January 2019:
...
Alternatively, we can use SITC codes - here, we load all live beverages (SITC2 code 11):
...
Note that these need to be in character format, and include any leading zeros. This is because, unlike HS or CN codes, odd SITC codes exist (e.g. SITC1). We can also select a selection of codes in a similar way to HS/CN codes:

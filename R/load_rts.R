...
#' @param flow The trade flow to be loaded. Takes one or more integers (3 and/or 4), where 3 is imports and 4 is exports. Defaults to NULL (all flows). Note: unlike `load_ots()`, RTS data is only available for flow codes 3 and 4 - flows 1 and 2 will always return zero rows. The FlowType lookup labels these "Non-EU Imports"/"Non-EU Exports", but for RTS this label is misleading: these codes actually represent *all* imports/exports (EU and non-EU combined), not just non-EU trade.
...
load_rts <- function(month = NULL,
  ...
  check_month_arg(month)
  check_flow_arg(flow, valid = 3:4)
  check_country_arg(country)
  check_region_arg(region)
  check_uk_country_arg(uk_country)
  check_output_arg(output)
  check_logical_arg(join_lookup, "join_lookup")
  ...
  # If the API returned no rows, there is nothing to join lookups onto - and
  # the expected join columns (e.g. FlowTypeId) won't exist on an empty
  # result, so attempting the joins below would fail. check_status() has
  # already warned about the empty result via load_custom(), so just return
  # it as-is:
  if(nrow(rts_data) == 0){ return(rts_data) }
  ...
  if(join_lookup == FALSE) { return(rts_data) } else {
    ...
    # NOTE: RTS data only ever contains FlowTypeId 3 and 4. The FlowType
    # lookup below returns "Non-EU Imports"/"Non-EU Exports" as the label
    # for these codes (as it does for load_ots()'s OTS data), but for RTS
    # this label is misleading - these codes actually cover *all* imports/
    # exports (EU and non-EU combined). See check_flow_arg()'s call site
    # above (valid = 3:4) and this function's `flow` documentation.
